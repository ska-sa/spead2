...
def get_args():
    ...
    group.add_argument(
        "--items", type=int, default=1, help="Number of items per heap [%(default)s]"
    )
    ...
    group.add_argument(
        "--descriptors", type=int, help="Description issue frequency [only at start]"
    )
    ...
    for i, is_end in enumerate(rep):
        ...
        if is_end:
            ...
            heap = item_group.get_heap(
                descriptors="all" if i < n_substreams else "stale", data="all"
            )
...
async def async_main():
    ...
    if heap_size != args.heap_size:
        logging.warn(
            "Heap size is not an exact multiple: using %d instead of %d", heap_size, args.heap_size
        )
    item_group = spead2.send.ItemGroup(
        descriptor_frequency=args.descriptors, flavour=sender.make_flavour()
    )
