...
tcp_reader::tcp_reader(
    ...
    max_size(max_size),
    buffer(new std::uint8_t[max_size * mmsg_count]),
    buffer2(new std::uint8_t[max_size * mmsg_count])
{
...
bool tcp_reader::process_buffer(const std::size_t bytes_recv)
{
    ...
    while (bytes_avail >= pkt_size)
    {
        ...
        if (parse_packet(bytes_avail)) {
            // we don't enqueue any more reads, as the heap actually finished
            LOG_STEP("Heap finished");
            return false;
        }
...
bool tcp_reader::parse_pkt_size(std::size_t &bytes_avail)
{
...
void tcp_reader::enqueue_receive()
{
    ...
    peer.async_receive(
        boost::asio::buffer(buffer.get(), max_size * mmsg_count),
