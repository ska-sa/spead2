...
AC_DEFUN([SPEAD2_CHECK_FEATURE], [
    ...
    AC_CACHE_CHECK([for $2], [cv], [
        spead2_save_CXXFLAGS="$CXXFLAGS"
        spead2_save_LIBS="$LIBS"
        m4_ifval([$9], [CXXFLAGS="$9 $CXXFLAGS"])
        m4_ifval([$10], [LIBS="$10 $LIBS"])
...
#                  [EXTRA-PREAMBLE], [EXTRA-CXXFLAGS], [EXTA-LIBS])
# -----------------------------------------------------------------------------
AC_DEFUN([SPEAD2_CHECK_LIB], [SPEAD2_CHECK_FEATURE([lib_$2], [-l$2], [$1], [$2], [$3], [$4], [$5], [$6], [$7], [$8])])

# SPEAD2_CHECK_HEADER(HEADER, LIBRARY, BODY, ACTION-IF-FOUND, ACTION-IF-NOT-FOUND
#                     [EXTRA-PREAMBLE], [EXTRA-CXXFLAGS], [EXTA-LIBS])
# -----------------------------------------------------------------------------
AC_DEFUN([SPEAD2_CHECK_HEADER], [SPEAD2_CHECK_FEATURE([header_$1], [$1], [$1], [$2], [$3], [$4], [$5], [$6], [$7], [$8])])
