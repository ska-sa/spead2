# Copyright 2015, 2019-2021, 2023-2024 National Research Foundation (SARAO)
#
...
class Flavour(spead2.Flavour):
    ...
    def make_header(self, num_items):
        ...
        return struct.pack(
            ">Q", 0x5304000000000000 | (address_size << 32) | (item_size << 40) | num_items
        )
    ...
    def make_shape(self, shape):
        ...
        assert not (
            self.bug_compat & (spead2.BUG_COMPAT_DESCRIPTOR_WIDTHS | spead2.BUG_COMPAT_SHAPE_BIT_1)
        )
        ans = []
        for size in shape:
            if size < 0:
...
class TestEncode:
    ...
    def test_lifetime(self):
        ...
        item = spead2.Item(
            id=0x2345, name="name", description="description", shape=(2, 3), dtype=np.uint16
        )
    ...
    def test_numpy_simple(self):
        ...
        item = spead2.Item(
            id=id, name="name", description="description", shape=shape, dtype=np.uint16
        )
        item.value = data
    ...
    def test_numpy_fortran_order(self):
        ...
        item = spead2.Item(
            id=id, name="name", description="description", shape=shape, dtype=np.uint16, order="F"
        )
        item.value = data
        packet = self.flavour.items_to_bytes([item])
        assert hexlify(packet) == hexlify(expected)
    ...
    def test_fallback_types(self):
        ...
        item = spead2.Item(
            id=id, name="name", description="description", shape=shape, format=format
        )
        item.value = data
        packet = self.flavour.items_to_bytes([item])
        assert hexlify(packet) == hexlify(expected)
    ...
    def test_small_fixed(self):
        ...
        item = spead2.Item(
            id=id, name="name", description="description", shape=(), format=[("u", 16)]
        )
        item.value = data
        packet = self.flavour.items_to_bytes([item], [])
        assert hexlify(packet) == hexlify(expected)
    ...
    def test_small_variable(self):
        ...
        item = spead2.Item(
            id=id, name="name", description="description", shape=shape, format=[("u", 8)]
        )
        item.value = data
        packet = self.flavour.items_to_bytes([item], [])
        assert hexlify(packet) == hexlify(expected)
    ...
    def test_numpy_zero_length(self):
        ...
        with pytest.raises(ValueError):
            spead2.Item(id=0x2345, name="name", description="description", shape=(), dtype=np.str_)

    def test_fallback_zero_length(self):
        """A zero-length type raises :exc:`ValueError`"""
        with pytest.raises(ValueError):
            spead2.Item(
                id=0x2345, name="name", description="description", shape=(), format=[("u", 0)]
            )
    ...
    def test_replicate_pointers(self):
        ...
        item2 = spead2.Item(
            ...
            shape=(),
            format=[("u", self.flavour.heap_address_bits)],
            value=0xDEADBEEF,
        )
        expected = [
            b"".join(
        ...
        packets = self.flavour.items_to_bytes(
            [item1, item2], [], max_packet_size=72, repeat_pointers=True
        )
...
class TestStream:
    def setup_method(self):
        ...
        self.stream = send.BytesStream(
            spead2.ThreadPool(), send.StreamConfig(rate=1e6, max_heaps=2)
        )
        # A large heap
    ...
    def test_send_explicit_cnt(self):
        ...
        self.stream.send_heap(ig.get_start())
        self.stream.send_heap(ig.get_start())
        expected_cnts = [1, 0x1111111111, 0x9876543210AB, 0x2345623456, 2**48 - 1, 0]
        expected = b""
    ...
    def test_round_robin(self):
        ...
        for i in range(n):
            ...
            igs[i].add_item(
                0x1000 + i, "test", "Test item", shape=data.shape, dtype=data.dtype, value=data
            )
        self.stream = send.BytesStream(
            spead2.ThreadPool(), send.StreamConfig(max_heaps=n, max_packet_size=88)
        )
        self.stream.send_heaps(
